...
def init():
    ...
    else: print("\033[91m error:\033[0m unsupported shell\nUse bash, zsh or fish")

    env_var = dedent(f"""
        # Automatically generated by 'declan init'
        export DECLAN_CONFIG_PATH='{cfg_path}'"""
    )
    with open(f"/home/{user}/{sh_cfg_path}", "a+") as s, open(f"/home/{user}/.bashrc", "a+") as b:
        s.seek(0); b.seek(0)
        if "DECLAN_CONFIG_PATH" in s.read() or "DECLAN_CONFIG_PATH" in b.read(): raise EnvVarExistsError(sh_path)
        else:
            s.write(env_var)
            if sh_cfg_path != ".bashrc": b.write(env_var)
            print(" \033[3m(exported as $DECLAN_CONFIG_PATH)\033[0m", end="\n")



    print(
        "\033[92m\n Initialization complete.\033[0m\n"
        "\033[91m Start a new shell before running operations!\033[0m\n"
...
def clear(cfg_path):
    ...
    if input("\033[35m::\033[0m \033[0mContinue? [Y/n]: ").strip().lower() == "y":
        ...
        if input("\nEnvironment variable: $DECLAN_CONFIG_PATH\n\033[35m::\033[0m \033[1mWipe environment variable? [Y/n]:\033[0m ").strip().lower() == "y":
            ...
            with open(f"/home/{user}/{sh_cfg_path}", "r") as s, open(f"/home/{user}/.bashrc", "r") as b:
                sh_cfg_lines = s.readlines()
                bashrc_lines = b.readlines()

            for i, line in enumerate(sh_cfg_lines):
                if "# Automatically generated by 'declan" in line:
                    s_lines_to_rmv = [i, i + 1]
            for i, line in enumerate(bashrc_lines):
                if "# Automatically generated by 'declan" in line:
                    b_lines_to_rmv = [i, i + 1]
            
            with open(f"/home/{user}/{sh_cfg_path}", "w") as s, open(f"/home/{user}/.bashrc", "w") as b:
                for i, line in enumerate(sh_cfg_lines):
                    if i not in s_lines_to_rmv:
                        s.write(line)
                for i, line in enumerate(bashrc_lines):
                    if i not in b_lines_to_rmv:
                        b.write(line)
...
def relay_rebuild(packages, services):
    ...
    if args.operation == "relay":
        ...
        if pending_updates.stdout:
            to_install = "updates" # make sure to_install exists
    ...
    if to_remove:
        if args.casc:
            ...
            run(["yay", "-Rns", "--noconfirm", "--noprogressbar", *to_remove])
...
def cache_config(user, home_cfg_path):
    ...
    copy2(home_cfg_path, cache_path / (date_last_mod + ".json")) # e.g. 2026-07-12_14-41.json
    ...
    if args.operation in ["relay", "rebuild"]:
        if which("yay") is None:
            ...
            if get_yay_yn == "y":
                ...
                run(
                    "git clone https://aur.archlinux.org/yay-bin.git && cd yay-bin && makepkg -si --noconfirm --noprogressbar",
                    shell=True,
